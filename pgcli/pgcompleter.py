...
class PGCompleter(Completer):
    ...
    def alias(self, tbl, tbls):
        ...
        if self.generate_aliases:
            # All upper-case letters in the table name, or, if there are no
            # upper-case letters, the first letter + all letters preceded by _
            t = self.unescape_name(tbl)
            tbl = ''.join([l for l in t if l.isupper()]
                or [l for l, prev in zip(t,  '_' + t)
                    if prev == '_' and l != '_'])
    ...
    def _should_alias(self, suggestion):
        return (self.generate_aliases
            and type(suggestion).__name__.startswith('Aliasable'))
    ...
    def get_function_matches(self, suggestion, word_before_cursor):
        if suggestion.filter == 'for_from_clause':
            ...
            if self._should_alias(suggestion):
    ...
    suggestion_matchers = {
        AliasableTable: get_table_matches,
        AliasableView: get_view_matches,
        AliasableFunction: get_function_matches,
